...
-(void)createTitleBar
{
    UIView *titleBarView=[[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kTitleBarHeight)];
    [self.view addSubview:titleBarView];
    titleBarView.backgroundColor=Color_BG_BLACK;

    
    CGFloat btnWidth=kTitleBarHeight -30;
    CGFloat btnHeight=kTitleBarHeight -30;
    
    UIImage *bgImage = [UIImage imageNamed:@"back_arrow.png"];
    UIImage *bgImage_ = [UIImage imageNamed:@"back_arrow_.png"];
    UIButton *leftButton=[UIButton buttonWithType:UIButtonTypeCustom];
    [leftButton setFrame:CGRectMake(0, 20, btnWidth, btnHeight)];
    [leftButton setImage:bgImage forState:UIControlStateNormal];
    [leftButton setImage:bgImage_ forState:UIControlStateHighlighted];
    [titleBarView addSubview:leftButton];
    [leftButton addTarget:self action:@selector(leftAction) forControlEvents:UIControlEventTouchUpInside];
    leftButton.center=CGPointMake(leftButton.center.x, 20+(kTitleBarHeight-20)/2.0);
    
    
    UILabel *titleLabel=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-200, 30)];
    [titleBarView addSubview:titleLabel];
    titleLabel.center=CGPointMake(SCREEN_WIDTH/2.0, leftButton.center.y);
    titleLabel.textColor=[UIColor whiteColor];
    titleLabel.textAlignment=NSTextAlignmentCenter;
    titleLabel.font=GETFONT(20);
    titleLabel.text=@"相册";
    
    
    
    UIButton *rightButton=[UIButton buttonWithType:UIButtonTypeCustom];
    [rightButton setFrame:CGRectMake(SCREEN_WIDTH-50, 20, 50, 50)];
//    [rightButton setImage:GETIMG(@"add_icon_logo") forState:UIControlStateNormal];
//    [rightButton setImage:GETIMG(@"add_icon_logo") forState:UIControlStateHighlighted];
    ...
    rightButton.center=CGPointMake(rightButton.center.x, 20+(kTitleBarHeight-20)/2.0);
...
- (void)configBottomToolBar {
    ...
    NSString *previewText = [NSBundle tz_localizedStringForKey:@"预览"];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"最多选择 %d 张图片 "], tzImagePickerVc.maxImagesCount];
